...
Unified firmware and software flasher in one app, with two clearly separate tabs. The **Firmware** tab handles hardware — ESP32 (Marauder, GhostESP, Bruce, and more) plus Raspberry Pi SD images. The new **Software** tab flashes PC/USB operating systems — Kali Linux, Tails OS, Arch, and more — to USB, integrity-verified by sha256/signature. An auto-updating firmware/OS catalog keeps versions current, while full offline use works from a cached catalog and downloaded images with no internet; the app also self-updates. In-app tooltips on every control and a thorough How-To / tutorial tab.
> In progress: building out the Software tab and the auto-updating + offline catalog. Strictly the flasher — no controller, logger, or wardriving (those live in Cyber Controller).
