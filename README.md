...
The all-in-one security hardware controller for cyberdecks and field deployments. Flash, control, and coordinate ESP32 (Marauder, Bruce, GhostESP), Flipper Zero, and Raspberry Pi security hardware from one dashboard. Four native UIs (PyQt5, Tkinter, Textual TUI, and a Flask + SocketIO web remote), 42 firmware profiles across five flash backends, thirteen protocol-aware serial parsers, multi-device wardriving into one merged WiGLE map, a unified action broadcast that fires one intent across every connected radio at once, cross-device target sharing with auto-routing, and a red-team-audited security core (multiple convergent security reviews — all findings closed) with scrypt auth, AES-256-GCM, and SSRF hardening. Released at v1.6.9; ARM + x64, Windows + Linux + macOS.
