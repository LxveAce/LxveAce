...
The all-in-one security hardware controller for cyberdecks and field deployments. Flash, control, and coordinate ESP32 (Marauder, Bruce, GhostESP), Flipper Zero, and Raspberry Pi security hardware from one dashboard. Four native UIs (PyQt5, Tkinter, Textual TUI, and a Flask + SocketIO web remote), 19 firmware profiles across five flash backends, eight protocol-aware serial parsers, a unified action broadcast that fires one intent across every connected radio at once, cross-device target sharing with auto-routing, and a red-team-audited security core (full security audit — all findings closed) with scrypt auth, AES-256-GCM, and SSRF hardening. Released at v1.2.1; ARM + x64, Windows + Linux + macOS.
> Shipped in v1.2.1: a Software-OS flashing tab (flash verified Kali / Tails / Arch to USB — auto-resolved latest version with an offline bundled fallback, SHA-256 + OpenPGP verified); a Wardriving tab (GPS-tagged Wi-Fi → WiGLE CSV, for lawful owner-authorized use); in-app Access-Gate setup (Settings → Access Gate: set admin password / physical USB key / policy from the GUI, salted-scrypt + encrypted vault); and a How-To tab. The prior Windows one-click .exe startup crash is fixed and verified. A weekly CI job keeps the bundled OS catalog current, and everything works fully offline.
